...
export const getData = (part2 = false) => {
  const qSet = data
    .trim()
    .split("\n\n")
    .map((question) => {
      const [aStr, bStr, pStr] = question.split("\n");
      const [ax, ay] = aStr
        .trim()
        .split(":")[1]
        .trim()
      ...
      const [px, py] = pStr
        ...
        .map((s) =>
          part2
            ? Number(s.split("=")[1]) + 10000000000000
            : Number(s.split("=")[1])
        );
      return { A: { x: ax, y: ay }, B: { x: bx, y: by }, P: { x: px, y: py } };
    });
